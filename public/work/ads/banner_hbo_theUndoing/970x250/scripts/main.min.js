...
var $ad, $bgExits, $videoContainer, $mainVideo, $videoExit, $vidControls,
$background, $lockup, $date, $logo, $$replayBtn;
// $ctaTxt, $cta, $cfs,
...
AD.adReady = function() {
    ...
    // debug
    if (debug) $mainVideo.currentTime = debugTime;
};
...
AD.setVars = function () {
    ...
    // $cfs                = getEl ('snd-icon');
    $endframe           = getEl ('endframe');
    $date               = getEl ('date');
    $logo               = getEl ('hbomax');
    $lockup             = getEl ('dateLockup');
    // $ctaTxt             = getEl ('ctaTxt');
    // $cta                = getEl ('cta');
    $background         = getEl ('background');
    $$replayBtn         = getEl ('replayBtn');
};
...
AD.addListeners = function () {
    ...
    // $cta.addEventListener('click', AD.exitHandler);
...
AD.addVideos = function() {
    ...
    // default
    BLT.Video.addVideo (pathAka+'HBO_Undoing_ROS_15s_970x250_noTuneIn', 'default', 'intro');
    BLT.Video.addVideo (pathAka+'hbo_undoing_970x250_30_unnerve_PRE', 'default', '30sec');

    // tonight
    BLT.Video.addVideo (pathAka+'HBO_Undoing_ROS_15s_970x250_noTuneIn', 'tonight', 'intro');
    BLT.Video.addVideo (pathAka+'hbo_undoing_970x250_30_unnerve_TON', 'tonight', '30sec');

    // now
    BLT.Video.addVideo (pathAka+'HBO_Undoing_ROS_15s_970x250_noTuneIn', 'now', 'intro');
    BLT.Video.addVideo (pathAka+'hbo_undoing_970x250_30_unnerve_NOW', 'now', '30sec');
};
...
AD.videoComplete = function(){
    ...
    if($initialPlay){
        ...
        addClass($videoContainer, "hidden");
        TweenLite.set($lockup, {autoAlpha:1});
...
AD.videoPrep = function() {
    $mainVideo.removeEventListener ('timeupdate', AD.checkPercent);
    TweenLite.set($$replayBtn, {autoAlpha:0});
    TweenLite.set($lockup, {autoAlpha:0});
...
AD.checkPercent = function () {
    ...
    if (vPercent > videoPercent) {
        TweenLite.to($videoContainer, 1, {opacity: 0});
    }
    if (vPercent > vp_lockup){
        tl.play();
    }
    // if (vPercent > vp_cfs){
    //     addClass($cfs,'hidden');
...
AD.endframeAnimation = function() {
    tl.addLabel("start", 0);
    tl.addLabel("replay", 1.5);
    tl.addLabel("end", 6);

    tl.from($lockup, 2,{autoAlpha: 0, ease: Power4.easeOut}, "start");
    tl.from($logo, 2,{ x: "-=8", rotation: .001,  ease: Power4.easeOut }, "start");
    tl.from($date, 2,{ x: "+=7", rotation: .001, ease: Power4.easeOut }, "start");

    tl.from($$replayBtn , 2, {autoAlpha:0, ease: Power1.easeOut},"replay");
}
...
AD.checkDate = function () {
    ...
    // BLT.Tunein.parseDates (2020, 10, 26); // USE TO TEST DATES
