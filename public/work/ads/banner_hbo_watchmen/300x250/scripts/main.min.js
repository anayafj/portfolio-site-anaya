...
var $ad, $bgExits, $videoContainer, $mainVideo, $videoExit, $cfs, $video_tt, $vidControls,
    $background, $date, $cta, $lockup, $replayBtn;
...
AD.initAd = function () {
    ...
    TweenLite.set($replayBtn, {autoAlpha:0});
...
AD.addVideos = function() {
    ...
    // default
    BLT.Video.addVideo (pathAka+"wm_300x250_intro", 'default', 'intro');
    BLT.Video.addVideo (pathAka+"wm_300x250_user_OCT20", 'default', '30sec');

    // // sunday
    BLT.Video.addVideo (pathAka+"wm_300x250_intro", 'sunday', 'intro');
    BLT.Video.addVideo (pathAka+"wm_300x250_user_SUNDAY", 'sunday', '30sec');

    // // tomorrow
    BLT.Video.addVideo (pathAka+"wm_300x250_intro", 'tomorrow', 'intro');
    BLT.Video.addVideo (pathAka+"wm_300x250_user_TOMORROW", 'tomorrow', '30sec');

    // // tonight
    BLT.Video.addVideo (pathAka+"wm_300x250_intro", 'tonight', 'intro');
    BLT.Video.addVideo (pathAka+"wm_300x250_user_TONIGHT", 'tonight', '30sec');

    // // sundays
    BLT.Video.addVideo (pathAka+"wm_300x250_intro", 'sundays', 'intro');
    BLT.Video.addVideo (pathAka+"wm_300x250_user_SUNDAYS", 'sundays', '30sec');

    // // post-tonight
    BLT.Video.addVideo (pathAka+"wm_300x250_intro", 'post-tonight', 'intro');
    BLT.Video.addVideo (pathAka+"wm_300x250_user_POST_TONIGHT", 'post-tonight', '30sec');
...
AD.videoComplete = function(){
    ...
    if($initialPlay){
        ...
        TweenLite.to($videoContainer,1, {opacity:0, ease: Expo.easeOut, onComplete:function(){
            ...
            addClass($videoContainer,'user-video');
            TweenLite.to($replayBtn, 1, {autoAlpha:1, ease: Expo.easeOut});
        }});
    } else {
        console.log("video Complete - User Video");
        addClass($videoContainer, "hidden");
        TweenLite.to($replayBtn, 1, {autoAlpha:1, ease: Expo.easeOut});
        removeClass($lockup, "hidden");
        tl.seek('end');
    }
};
...
AD.killVideo = function(){
    ...
    removeClass($lockup, "hidden");
    TweenLite.set($replayBtn, {autoAlpha:1});
    addClass($videoContainer, "hidden");
    tl.seek('end');
    BLT.Video.killVideo();
};
...
AD.animFunc = function(){
    ...
    tl.from($date,1,{opacity:0 }, "lockup");
    tl.from($cta,1,{opacity:0 }, "lockup+=.5");
};
...
AD.checkDate = function () {
    ...
    // BLT.Tunein.parseDates (2019, 10, 28); // USE TO TEST DATES
