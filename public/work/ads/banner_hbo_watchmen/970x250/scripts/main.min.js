...
var $ad, $bgExits, $videoContainer, $mainVideo, $videoExit, $cfs, $video_tt, $vidControls,
    $background, $date, $replayBtn;
...
AD.adReady = function() {
    ...
    if (debug) $mainVideo.currentTime =  skip_video; // for testing only
};
...
AD.setVars = function () {
    ...
    $cfs                = getEl ('snd-icon');
    $video_tt           = getEl ('video-tt');
    $background         = getEl ('background');
    $date               = getEl ('date-img');
    $replayBtn          = getEl ('replay-container');
};
...
AD.addListeners = function () {
    ...
    $replayBtn.addEventListener('click', AD.replayVideo);
...
AD.exitTriggered = function(event){ AD.killVideo()};
...
AD.addVideos = function() {
    ...
    // default
    BLT.Video.addVideo (pathAka+"wm_970x250_OCT20", 'default', 'intro');
    BLT.Video.addVideo (pathAka+"wm_444x250_user_OCT20", 'default', '30sec');

    // // sunday
    BLT.Video.addVideo (pathAka+"wm_970x250_SUNDAY", 'sunday', 'intro');
    BLT.Video.addVideo (pathAka+"wm_444x250_user_SUNDAY", 'sunday', '30sec');

    // // tomorrow
    BLT.Video.addVideo (pathAka+"wm_970x250_TOMORROW", 'tomorrow', 'intro');
    BLT.Video.addVideo (pathAka+"wm_444x250_user_TOMORROW", 'tomorrow', '30sec');

    // // tonight
    BLT.Video.addVideo (pathAka+"wm_970x250_TONIGHT", 'tonight', 'intro');
    BLT.Video.addVideo (pathAka+"wm_444x250_user_TONIGHT", 'tonight', '30sec');

    // // sundays
    BLT.Video.addVideo (pathAka+"wm_970x250_SUNDAYS", 'sundays', 'intro');
    BLT.Video.addVideo (pathAka+"wm_444x250_user_SUNDAYS", 'sundays', '30sec');

    // // post-tonight
    BLT.Video.addVideo (pathAka+"wm_970x250_POST_TONIGHT", 'post-tonight', 'intro');
    BLT.Video.addVideo (pathAka+"wm_444x250_user_POST_TONIGHT", 'post-tonight', '30sec');
...
AD.videoComplete = function(){
    ...
    if($initialPlay){
        console.log("video Complete - Initial Video");
        $initialPlay = false;
        TweenLite.to($videoContainer,1, {opacity:0, ease: Expo.easeOut, onComplete:function(){
            addClass($videoContainer, "hidden");
        addClass($videoContainer,'user-video');
        removeClass($replayBtn, "hidden");
        }});
    } else {
        console.log("video Complete - User Video");
        addClass($videoContainer, "hidden");
        removeClass($replayBtn, "hidden");
    }
};
...
AD.checkPercent = function () {
    ...
    if(vPercent > introVideoDone){
        console.log("Remove intro TT and CFS");
        addClass($cfs, 'hidden');
        addClass($video_tt, 'hidden');
        $mainVideo.removeEventListener ('timeupdate', AD.checkPercent);
    }
...
// AD.animFunc = function(){
//     tl.addLabel('start', 0);
//     tl.addLabel('TT', .5);
//     tl.addLabel('lockup', 2);
//     tl.addLabel('end', 6);

//     tl.from($background,2,{scale: 1.4, rotateX: 0.001, ease: Expo.easeOut }, "start");
//     tl.from($TT,2,{scale: 1.5, opacity:0, rotateX: 0.001, ease: Expo.easeOut }, "TT");
//     tl.from($header,3,{scale: 1.5, opacity:0, rotateX: 0.001, ease: Expo.easeOut }, "TT");
//     tl.from([$hboLogo, $cta, $tuneIn],1,{opacity:0 }, "lockup");
// };
...
AD.checkDate = function () {
    ...
    BLT.Tunein.addDate (2019, 10, 14, 'sunday');
    BLT.Tunein.addDate (2019, 10, 19, 'tomorrow');
    BLT.Tunein.addDate (2019, 10, 20, 'tonight');
    BLT.Tunein.addDate (2019, 10, 21, 'sundays');
    BLT.Tunein.addDate (2019, 10, 27, 'post-tonight');
    BLT.Tunein.addDate (2019, 10, 28, 'sundays');
    BLT.Tunein.addDate (2019, 11, 03, 'post-tonight');
    BLT.Tunein.addDate (2019, 11, 04, 'sundays');
    BLT.Tunein.addDate (2019, 11, 10, 'post-tonight');
    BLT.Tunein.addDate (2019, 11, 11, 'sundays');
    BLT.Tunein.addDate (2019, 11, 17, 'post-tonight');
    BLT.Tunein.addDate (2019, 11, 18, 'sundays');
    BLT.Tunein.addDate (2019, 11, 24, 'post-tonight');
    BLT.Tunein.addDate (2019, 11, 25, 'sundays');
    BLT.Tunein.addDate (2019, 12, 01, 'post-tonight');
    BLT.Tunein.addDate (2019, 12, 02, 'sundays');
    BLT.Tunein.addDate (2019, 12, 08, 'post-tonight');

    BLT.Tunein.parseDates ();
    // BLT.Tunein.parseDates (2019, 10, 27); // USE TO TEST DATES


    switch (BLT.Tunein.currentID) {

        case 'sunday':
            console.info('DATE - sunday');
            addClass($date, 'date2');
            break;

        case 'tomorrow':
            console.info('DATE - tomorrow');
            addClass($date, 'date3');
            break;
        ...
        case 'sundays':
            console.info('DATE - sundays');
            addClass($date, 'date5');
            break;

        case 'post-tonight':
            console.info('DATE - post-tonight');
            addClass($date, 'date6');;
            break;
